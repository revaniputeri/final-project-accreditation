...
<body>
    ...
    <h3 class="text-center">DATA PORTOFOLIO DOSEN - PROFESI</h3>

    <table class="border-all" style="font-size: 11pt;">
        <thead>
            <tr>
                <th>No</th>
                <th>ID Profesi</th>
        ...
        <tbody>
            @foreach ($profesi as $key => $item)
                <tr>
                    ...
                    <td>
                        @if ($item['bukti'])
                            <a href="{{ url('storage/portofolio/profesi/' . $item['bukti']) }}" target="_blank">Lihat
                                File</a>
                        @else
                            Tidak ada file
                        @endif

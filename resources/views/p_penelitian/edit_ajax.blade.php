<form id="formEditPenelitian" method="POST" action="{{ route('p_penelitian.update_ajax', $penelitian->id_penelitian) }}" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    ...
    <div class="modal-body">
        ...
        <div class="mb-3">
            ...
            @if($penelitian->bukti)
                <small class="text-muted">File saat ini: <a href="{{ asset('storage/p_penelitian/' . $penelitian->bukti) }}" target="_blank">{{ $penelitian->bukti }}</a></small>
            @endif
        </div>
    </div>

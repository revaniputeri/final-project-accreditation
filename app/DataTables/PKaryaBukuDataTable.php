...
class PKaryaBukuDataTable extends DataTable
{
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        return (new EloquentDataTable($query))
            ...
            ->addColumn('aksi', function ($row) use ($user, $isDos, $isAdm) {
                $buttons = [];
                $detailUrl = route('portofolio.karya_buku.detail_ajax', $row->id_karya_buku);
                ...
                if ($isDos) {
                    $validasiUrl = route('portofolio.karya_buku.validasi_ajax', $row->id_karya_buku);
                ...
                if ($isDos || $isAdm) {
                    $editUrl = route('portofolio.karya_buku.edit_ajax', $row->id_karya_buku);
                    $deleteUrl = route('portofolio.karya_buku.confirm_ajax', $row->id_karya_buku);
            ...
            ->setRowId('id_karya_buku');
    }

    public function query(PKaryaBukuModel $model): QueryBuilder
    {
        /** @var UserModel|null $user */
        $user = Auth::user();
        $query = $model->newQuery()
            ->select('p_karya_buku.*', 'profile_user.nama_lengkap')
            ->leftJoin('user', 'p_karya_buku.id_user', '=', 'user.id_user')
    ...
    public function getColumns(): array
    {
        ...
        $columns = [
            ...
            Column::make('judul_buku')->title('Judul Buku'),
    ...
    {
        return 'PKaryaBuku_' . date('YmdHis');
    }
}
